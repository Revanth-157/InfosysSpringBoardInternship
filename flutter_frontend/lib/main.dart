...
class _LoginPageState extends State<LoginPage> {
  ...
  String get _apiBase {
    ...
    if (Platform.isWindows || Platform.isLinux || Platform.isMacOS) return 'http://127.0.0.1:5000';
  ...
  Future<void> _login() async {
    ...
    try {
      final resp = await http.post(Uri.parse('$_apiBase/login'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'username': u, 'password': p})).timeout(Duration(seconds: 8));
      if (resp.statusCode == 200) {
        final body = jsonDecode(resp.body);
        final token = body['token'] as String?;
        if (token != null) {
          widget.onLogin(u, token);
          return;
        }
      }
      ...
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (_error.isNotEmpty) ...[SizedBox(height: 8), Text(_error, style: TextStyle(color: Colors.red))]
          ],
        ),
      ),
    );
...
class _RegisterPageState extends State<RegisterPage> {
  ...
  Future<void> _register() async {
    ...
    setState(() => _loading = true);
    try {
      final resp = await http.post(Uri.parse('$_apiBase/register'), headers: {'Content-Type': 'application/json'}, body: jsonEncode({'username': u, 'password': p})).timeout(Duration(seconds: 8));
      if (resp.statusCode == 200) {
...
class _HomePageState extends State<HomePage> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    // add an extra tab for dealer chat
  ...
  Future<void> _sendChatMessage({bool retrying = false}) async {
    ...
    try {
      ...
      // if the server indicates the context is missing, try to initialize once
      if (response.statusCode == 404 && !retrying) {
        final initResp = await http.post(
          Uri.parse('$_apiBase/chat/init/$jobId'),
          headers: {
            'Content-Type': 'application/json',
            if (_globalJwtToken != null && _globalJwtToken!.isNotEmpty)
              'Authorization': 'Bearer $_globalJwtToken',
        ...
        if (initResp.statusCode == 200) {
          // retry sending the original message once
          await _sendChatMessage(retrying: true);
          return;
        }
      }
  ...
  Widget _buildAnalysisView() {
    ...
    // log price estimation if present
    if (fullExtraction.containsKey('price_estimate')) {
      print('[DEBUG] price_estimate at top level: ${fullExtraction['price_estimate']}');
    }
    if (fullExtraction['vehicle_info'] is Map && (fullExtraction['vehicle_info'] as Map).containsKey('price_estimate')) {
      print('[DEBUG] price_estimate under vehicle_info: ${(fullExtraction['vehicle_info'] as Map)['price_estimate']}');
    }
    ...
    // Build price estimation card
    Widget _buildPriceEstimationCard() {
      // price_estimate may be top-level or nested under vehicle_info depending on
      // whether extract_full_lease_fields was used. handle both.
      Map<String, dynamic>? priceData;
      if (fullExtraction.containsKey('price_estimate')) {
        priceData = (fullExtraction['price_estimate'] as Map<String, dynamic>?);
      }
      if ((priceData == null || priceData.isEmpty) && fullExtraction['vehicle_info'] is Map) {
        priceData = (fullExtraction['vehicle_info'] as Map<String, dynamic>)['price_estimate'] as Map<String, dynamic>?;
      }
      if (priceData == null || priceData.isEmpty) return SizedBox.shrink();
      
      final source = priceData['source'] ?? 'unknown';
      final estimatedValue = priceData['estimated_value'];
      final dealerRetail = priceData['dealer_retail'];
      final privateParty = priceData['private_party'];
      final estimatedPayment = priceData['estimated_payment_monthly'];
      final note = priceData['note'] ?? '';
      
      return _sectionCard(
        '💰 Price Estimation (via $source)',
        Icons.trending_up,
        Colors.cyan[700]!,
        [
          if (estimatedValue != null) ...[
            _infoRow('Estimated Market Value', '\$${(estimatedValue is num ? estimatedValue : double.tryParse(estimatedValue.toString()) ?? 0).toStringAsFixed(2)}', valueColor: Colors.green[700], prefixIcon: Icons.attach_money),
          ],
          if (dealerRetail != null) ...[
            _infoRow('Dealer Retail Value', '\$${(dealerRetail is num ? dealerRetail : double.tryParse(dealerRetail.toString()) ?? 0).toStringAsFixed(2)}', prefixIcon: Icons.store),
          ],
          if (privateParty != null) ...[
            _infoRow('Private Party Value', '\$${(privateParty is num ? privateParty : double.tryParse(privateParty.toString()) ?? 0).toStringAsFixed(2)}', prefixIcon: Icons.person),
          ],
          if (estimatedPayment != null) ...[
            _infoRow('Est. Monthly Lease Payment', '\$${(estimatedPayment is num ? estimatedPayment : double.tryParse(estimatedPayment.toString()) ?? 0).toStringAsFixed(2)}', valueColor: Colors.blue[700], prefixIcon: Icons.calendar_today),
          ],
          if (note.isNotEmpty) ...[
            SizedBox(height: 8),
            Container(
              ...
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                SizedBox(width: 8),
                Expanded(child: Text(note, style: TextStyle(color: Colors.grey[700], fontSize: 12, height: 1.4))),
              ]),
            ),
          ],
        ],
        ...
        Expanded(
          child: _chatMessages.isEmpty
              ? Center(child: Text(contractForChat == null
                  ...
                  : 'Type a question below to start chatting about this contract.'))
              : ListView.builder(
      ...
      appBar: AppBar(
        ...
        bottom: TabBar(
          ...
          tabs: [
            ...
            Tab(icon: Icon(Icons.store), text: 'Dealers'),
          ],
        ),
      ),
      body: TabBarView(
        ...
        children: [
          ...
          // Dealers tab
          DealersChatPage(apiBase: _apiBase),
        ],
      ),
    );
  }
}
class DealersChatPage extends StatefulWidget {
  final String apiBase;
  DealersChatPage({required this.apiBase});
  @override
  _DealersChatPageState createState() => _DealersChatPageState();
}

class _DealersChatPageState extends State<DealersChatPage> {
  ...
  Future<void> _loadDealers() async {
    try {
      final resp = await http.get(Uri.parse('${widget.apiBase}/dealers'), headers: {'Authorization': 'Bearer ' + (_globalJwtToken ?? '')});
      if (resp.statusCode == 200) {
        final data = jsonDecode(resp.body);
        setState(() {
          _dealers = List<Map<String, dynamic>>.from((data['dealers'] as List? ?? []).map((d) => {'id': d['id'], 'username': d['username']}));
          _loading = false;
        });
      } else {
        setState(() => _loading = false);
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, i) {
                    ...
                    return ListTile(
                      ...
                      onTap: () => Navigator.of(context).push(MaterialPageRoute(builder: (_) => DealerConversationPage(dealerId: d['id'], dealerName: d['username'], apiBase: widget.apiBase))),
                    );
                  },
                ),
    );
...
class DealerConversationPage extends StatefulWidget {
  final int dealerId;
  final String dealerName;
  final String apiBase;
  DealerConversationPage({required this.dealerId, required this.dealerName, required this.apiBase});
  @override
  _DealerConversationPageState createState() => _DealerConversationPageState();
}

class _DealerConversationPageState extends State<DealerConversationPage> {
  ...
  void initState() {
    super.initState();
    _loadMessages();
    // Poll for new messages periodically with a shorter interval
    _pollTimer = Timer.periodic(Duration(milliseconds: 1500), (_) {
      if (!mounted) return;
      if (!_loading) _loadMessages();
    });
  }

  Future<void> _loadMessages() async {
    if (_loading) return; // avoid overlapping
    setState(() => _loading = true);
    try {
      final resp = await http.get(Uri.parse('${widget.apiBase}/messages/${widget.dealerId}'), headers: {'Authorization': 'Bearer ' + (_globalJwtToken ?? '')});
      if (resp.statusCode == 200) {
        final data = jsonDecode(resp.body);
        setState(() {
  ...
  Future<void> _send() async {
    ...
    _controller.clear();
    try {
      final resp = await http.post(Uri.parse('${widget.apiBase}/send-message'), headers: {'Content-Type': 'application/json', 'Authorization': 'Bearer ' + (_globalJwtToken ?? '')}, body: jsonEncode({'receiver_id': widget.dealerId, 'content': txt}));
      if (resp.statusCode == 200) {
        await _loadMessages();
      } else if (resp.statusCode == 403) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Cannot send: role restriction')));
      } else {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Chat with ${widget.dealerName}')),
      body: Column(children: [
        if (_loading) LinearProgressIndicator(minHeight: 3) else SizedBox(height: 3),
        Expanded(
          child: _messages.isEmpty
              ? Center(child: Text(_loading ? 'Loading messages...' : 'No messages yet'))
              : ListView.builder(
                  controller: _scrollController,
                  itemCount: _messages.length,
                  itemBuilder: (context, i) {
                    final m = _messages[i];
                    final isMine = m['is_mine'] ?? false;
                    return Align(
                      alignment: isMine ? Alignment.centerRight : Alignment.centerLeft,
                      child: Container(
                        margin: EdgeInsets.symmetric(horizontal: 12, vertical: 6),
                        padding: EdgeInsets.all(12),
                        decoration: BoxDecoration(color: isMine ? Colors.blue[200] : Colors.grey[300], borderRadius: BorderRadius.circular(8)),
        ...
        Padding(
          padding: EdgeInsets.all(8),
          child: Row(children: [
            Expanded(child: TextField(controller: _controller, decoration: InputDecoration(hintText: 'Type a message...'), onSubmitted: (_) => _send)),
