...
from vehicle_enrichment import extract_vehicle_info, extract_full_lease_fields, get_price_estimation
...
def _ensure_chat_context(job_id):
    """Ensure that a chat context exists for the given job_id.

    If the context is missing from the in‑memory CHATS map, attempt to
    recreate it by loading the lease record from the database and using
    its stored extraction data to seed the system prompt. Returns True if
    a context is available (either already existed or was just created);
    False if no matching lease could be found or initialization failed.
    """
    if job_id in CHATS:
        return True
    # try to load from DB
    db = SessionLocal()
    try:
        lease_row = db.query(Lease).filter(Lease.job_id == job_id).first()
        if not lease_row or not lease_row.extracted_json:
            return False
        try:
            payload = json.loads(lease_row.extracted_json)
        except Exception:
            return False
        full_extraction = payload.get('full_extraction', {})
        ...
        CHATS[job_id] = [{"role": "system", "content": system_prompt}]
        return True
    except Exception:
        return False
    finally:
...
class User(Base):
    ...
    role = Column(String(50), default='user', nullable=False)  # 'user' or 'dealer'
...
# Create default test users for convenience (only if not exists)
try:
    db = SessionLocal()
    if not db.query(User).filter(User.username == 'testuser').first():
        u = User(username='testuser', password_hash=generate_password_hash('testpass'), role='user')
        db.add(u)
        db.commit()
        print('[init_db] Created default test user: testuser / testpass (role: user)')
    if not db.query(User).filter(User.username == 'dealer').first():
        d = User(username='dealer', password_hash=generate_password_hash('dealerpass'), role='dealer')
        db.add(d)
        db.commit()
        print('[init_db] Created default test dealer: dealer / dealerpass (role: dealer)')
    db.close()
except Exception as e:
    print('[init_db] Could not create default test users:', e)
...
def get_user_role():
    ...
    if not user:
        return jsonify({'error': 'authentication required'}), 401
    return jsonify({'username': user.username, 'role': user.role, 'user_id': user.id})


@app.route('/dealers', methods=['GET'])
def get_dealers():
    """Get list of all dealers for user to select from"""
    user = get_user_from_request(request)
    if not user:
        return jsonify({'error': 'authentication required'}), 401
    ...
    try:
        ...
        return jsonify({
            'dealers': [{'id': d.id, 'username': d.username} for d in dealers]
        })
    finally:
        db.close()
...
def get_users():
    """Get list of all regular users (lessees) for dealers to select from"""
    user = get_user_from_request(request)
    if not user:
        return jsonify({'error': 'authentication required'}), 401
...
def send_message():
    """Send a message from user to dealer (or dealer to user)"""
    user = get_user_from_request(request)
    if not user:
        return jsonify({'error': 'authentication required'}), 401
    ...
    try:
        ...
        # Validate role restrictions: users can only message dealers, dealers can only message users
        if user.role == 'user' and receiver.role != 'dealer':
            return jsonify({'error': 'users can only message dealers'}), 403
        if user.role == 'dealer' and receiver.role != 'user':
            return jsonify({'error': 'dealers can only message users'}), 403
...
@app.route('/analyze-negotiation/<int:other_user_id>', methods=['GET'])
def analyze_negotiation(other_user_id):
    """Analyze conversation between user and dealer for negotiation tips (USER ONLY)"""
    user = get_user_from_request(request)
    if not user:
        return jsonify({'error': 'authentication required'}), 401
    
    if user.role != 'user':
        return jsonify({'error': 'only users can use negotiation analysis'}), 403
    ...
    try:
        ...
        conversation_text = '\n'.join([
            f"{'You' if m.sender_id == user.id else 'Dealer'}: {m.content}"
            for m in messages
        ])
        
        if not conversation_text.strip():
            return jsonify({'tips': [], 'summary': 'No conversation yet'})
        
        # Use Mistral to analyze negotiation
        try:
            analysis_prompt = (
                f"You are a car lease negotiation expert. Analyze this conversation between a lessee and a dealer. "
                f"Provide specific, actionable negotiation tips for the LESSEE (user) based on what the dealer is saying. "
                f"Format your response as a JSON object with 'tips' (array of strings) and 'summary' (string).\n\n"
                f"Conversation:\n{conversation_text}"
            )
            ...
            try:
                ...
                return jsonify({'tips': [analysis_text], 'summary': 'Analysis complete'})
        except Exception as e:
            print(f'[analyze_negotiation] Error analyzing conversation: {e}')
            return jsonify({'tips': ['Unable to analyze at this moment'], 'summary': 'Error'}), 500
    finally:
        db.close()
...
@app.route('/price_estimate', methods=['POST'])
def get_price_estimate():
    """Get price estimation for a vehicle given make, model, and year.
    ...
    Returns pricing data from available APIs (Edmunds, TrueCar) or heuristic fallback.
    """
    data = request.get_json()
    if not data:
    ...
    try:
        price_data = get_price_estimation(make, model, year)
        if price_data:
            return jsonify({'status': 'success', 'price_estimate': price_data})
        else:
...
@app.route('/chat/init/<job_id>', methods=['POST'])
def init_chat_for_job(job_id):
    """Explicitly initialize a chat context for a stored job.

    This endpoint may be called by the frontend when a user selects a
    previously saved contract. It attempts to rebuild the system prompt
    from the lease data cached in the database. The response mirrors the
    success/failure of creation but does not add any user message.
    """
    ok = _ensure_chat_context(job_id)
    if ok:
        return jsonify({'status': 'success', 'message': 'initialized'}), 200
    else:
        return jsonify({'error': 'not_found', 'message': 'Unable to locate lease for job_id'}), 404
...
def chat_for_job(job_id):
    ...
    """
    # if missing, try to rebuild context from database so stored contracts remain usable
    if job_id not in CHATS:
        if not _ensure_chat_context(job_id):
