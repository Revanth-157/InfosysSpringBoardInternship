...
def get_price_estimation(make: str, model: str, year: str, timeout: int = 10) -> Optional[Dict]:
    """Fetch price estimation data from available APIs (Edmunds, TrueCar, or fallback heuristics).
    
    Tries multiple price estimation APIs in order:
    1. Edmunds API (requires EDMUNDS_API_KEY env var)
    2. TrueCar API (requires TRUECAR_API_KEY env var)
    3. Fallback: estimate based on vehicle age and general market trends
    
    Returns a dict with keys:
      - source: which API provided the data (e.g., 'edmunds', 'truecar', 'heuristic')
      - estimated_value: estimated market value in USD
      - dealer_retail: dealer retail value if available
      - private_party: private party value if available
      - estimated_payment_monthly: estimated monthly lease payment
      - note: any notes about the estimate
    """
    if not make or not model or not year:
        return None
    ...
    result = {
        ...
        "dealer_retail": None,
        "private_party": None,
        "estimated_payment_monthly": None,
        "note": ""
    }

    # Try Edmunds API
    edmunds_key = os.getenv('EDMUNDS_API_KEY')
    if edmunds_key:
        try:
            # Edmunds API endpoint for vehicle details and pricing
            edmunds_url = f"https://api.edmunds.com/api/v2/vehicles/count?make={make}&model={model}&year={year}&fmt=json&api_key={edmunds_key}"
            resp = requests.get(edmunds_url, timeout=timeout)
            if resp.status_code == 200:
                data = resp.json()
                if data.get('count', 0) > 0:
                    # Try to get pricing info
                    pricing_url = f"https://api.edmunds.com/v1/vehicles/tmv?make={make}&model={model}&year={year}&api_key={edmunds_key}"
                    pricing_resp = requests.get(pricing_url, timeout=timeout)
                    if pricing_resp.status_code == 200:
                        pricing_data = pricing_resp.json()
                        if 'tmvs' in pricing_data and len(pricing_data['tmvs']) > 0:
                            tmv = pricing_data['tmvs'][0]
                            result['source'] = 'edmunds'
                            result['estimated_value'] = tmv.get('usedTmvRetail')
                            result['dealer_retail'] = tmv.get('usedTmvRetail')
                            result['private_party'] = tmv.get('usedTmvMsrp')
                            result['estimated_payment_monthly'] = tmv.get('msrp')
                            result['note'] = 'Pricing from Edmunds TMV'
                            return result
        except Exception as e:
            print(f"[price_estimation] Edmunds API error: {e}")

    # Try TrueCar API
    truecar_key = os.getenv('TRUECAR_API_KEY')
    if truecar_key:
        try:
            # TrueCar API for pricing estimates
            truecar_url = "https://api.truecar.com/v1/listings/estimate"
            params = {
                'make': make,
                'model': model,
                'year': year,
            ...
            resp = requests.get(truecar_url, params=params, timeout=timeout)
            if resp.status_code == 200:
                data = resp.json()
                if 'estimate' in data:
                    estimate = data['estimate']
                    result['source'] = 'truecar'
                    result['estimated_value'] = estimate.get('estimatedValue')
                    result['dealer_retail'] = estimate.get('retailPrice')
                    result['private_party'] = estimate.get('privatePartyPrice')
                    result['estimated_payment_monthly'] = estimate.get('estimatedMonthlyPayment')
                    result['note'] = 'Pricing from TrueCar'
                    return result
        except Exception as e:
            print(f"[price_estimation] TrueCar API error: {e}")
    ...
    try:
        ...
        base_value = 35000  # assume average new vehicle ~$35k
        
        # Adjust for age (rough depreciation: 15% year 1, 10% per year after)
        if age == 0:
            value = base_value
        elif age == 1:
            value = base_value * 0.85
        else:
            value = base_value * 0.85 * (0.90 ** (age - 1))
        
        value = max(value, 5000)  # Floor at $5k
        
        # Estimate monthly payment assuming 3-year lease at ~55% of value
        monthly_payment = (value * 0.55) / 36
        
        result['source'] = 'heuristic'
        result['estimated_value'] = round(value, 2)
        result['estimated_payment_monthly'] = round(monthly_payment, 2)
        result['note'] = f'Heuristic estimate based on {year} {make} {model} age ({age} years). API keys not configured.'
        return result
    except Exception as e:
        print(f"[price_estimation] Heuristic estimation error: {e}")
        return None
...
def extract_vehicle_info(text: str, use_nhtsa: bool = True) -> Dict:
    ...
    Also attempts to fetch price estimation data using available APIs.
    ...
    Returns a dict with keys:
      ...
      - price_estimate: price estimation dict with market value and payment estimates (if available)
    """
    result = {
        ...
        if use_nhtsa:
            ...
            if decoded:
                ...
                if make and model and year:
                    ...
                    if price_data:
                        result["price_estimate"] = price_data

    # If NHTSA decoding wasn't used or failed, still try price estimation with regex-extracted data
    if result["price_estimate"] is None and result["make"] and result["model"] and result["year"]:
        price_data = get_price_estimation(result["make"], result["model"], result["year"])
        if price_data:
            result["price_estimate"] = price_data
